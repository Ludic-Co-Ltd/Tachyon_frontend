...
export default (props) => {
	const { name, id } = props
	const link = Paths.showCaseStudy.replace(':id', id)
	...
	return (
		<Link to={link} className="mx-5 mb-5 bg-white shadow-md flex">
			...
			<div className="my-5 w-3/5">
				...
				<div className="font-bold text-primary text-lg break-words whitespace-pre-wrap">{name}{name}{name}{name}{name}{name}{name}{name}</div>
				<div className="text-xs text-gray-400 mb-1">72/100</div>
				<div className="text-xs text-gray-400">12月15日提出</div>
			</div>
		</Link>
	)
}